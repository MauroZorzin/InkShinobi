...
#pragma warning restore IDE0051
  ...
  System.Collections.IEnumerator RotateWorld(int direction) {
    ...
    int newIndex = (_currentRotationIndex + direction + Directions.Length) % Directions.Length;

    float elapsed = 0f;
    float startAngle = camPivot.eulerAngles.y;
  ...
  void HandleMovement() {
    ...
    float speedRatio = Mathf.Abs(_velocity.magnitude) / moveSpeed;
    float animValue = speedRatio > velocityMargin
        ? Mathf.Clamp(1f / speedRatio, 0f, 1f / velocityMargin)
        : 1f / velocityMargin;
