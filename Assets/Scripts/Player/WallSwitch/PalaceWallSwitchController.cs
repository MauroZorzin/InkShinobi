...
/// <summary>
/// Palace wall-switch state machine. Space enters or cancels aiming; primary mouse confirms
/// the current immutable evaluation only when it is valid.
/// </summary>
[DisallowMultipleComponent]
[RequireComponent(typeof(LineFollowController))]
public sealed class PalaceWallSwitchController : MonoBehaviour {
...
#pragma warning restore IDE0051
  ...
  public bool BeginAim() {
    ...
    currentEvaluation = EvaluateAtCursor();
    preview?.Show(currentEvaluation);
    if (verboseLogging) Debug.Log("[PalaceWallSwitch] Aim mode entered.", this);
    return true;
  }
  ...
  public bool TryConfirmCurrent() {
    ...
    if (!currentEvaluation.IsValid) {
      if (verboseLogging) Debug.Log($"[PalaceWallSwitch] Confirmation ignored: {currentEvaluation.FailureReason}.", this);
      return false;
    }
  ...
  private IEnumerator ExecuteSwitch(WallSwitchEvaluation evaluation) {
    ...
    currentEvaluation = WallSwitchEvaluation.Empty;
    state = SwitchState.Idle;
    if (verboseLogging) Debug.Log("[PalaceWallSwitch] Switch completed.", this);
  }
  ...
  private void TeleportToEvaluation(WallSwitchEvaluation evaluation) {
    ...
    PalaceLightExposure exposure = GetComponent<PalaceLightExposure>();
  ...
  private GameObject SpawnInk(GameObject prefab, Vector3 position, Vector3 direction) {
    ...
    try {
      ...
      Debug.LogError($"[PalaceWallSwitch] Could not instantiate an ink VFX. The switch will continue without it.\n{exception}", this);
      return null;
    }
