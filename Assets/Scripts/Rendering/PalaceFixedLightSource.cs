...
/// <summary>
/// Registers one fixed Palace light with the fullscreen monochrome composite. Its spherical
/// world-space field produces one continuous colored projection across every visible surface.
/// </summary>
[ExecuteAlways]
[DisallowMultipleComponent]
public sealed class PalaceFixedLightSource : MonoBehaviour {
  public const int MaximumVisibleLights = 8;

  private static readonly List<PalaceFixedLightSource> ActiveSources = new();
  ...
  [Tooltip("Strength of the yellow tint applied after the Palace monochrome pass.")]
  ...
  public static int FillShaderData(
    ...
    for (int i = ActiveSources.Count - 1; i >= 0 && count < MaximumVisibleLights; i--) {
      PalaceFixedLightSource source = ActiveSources[i];
      if (source == null) {
  ...
  /// <summary>
  /// Uses the same spherical fields sent to the Palace composite, allowing gameplay checks to
