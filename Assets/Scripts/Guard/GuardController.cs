...
public class GuardController : MonoBehaviour {
  ...
  [Header("Takedown")]
  [Tooltip("If enabled, destroy the guard GameObject on takedown instead of just disabling")]
  public bool destroyOnTakedown = false;
  public bool showStateLabel = false;
  // ── Public ────────────────────────────────────────────────────────────────
  ...
  private void SetState(GuardState newState) {
    ...
    switch (newState) {
      ...
      case GuardState.TakenDown:
        ...
        if (destroyOnTakedown) {
          Debug.Log($"[Guard] {name} is being destroyed (destroyOnTakedown flag is enabled).");
          Destroy(gameObject);
        }
        break;
    }
